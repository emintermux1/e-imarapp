...
REGISTRY: dict[str, SourceEntry] = {
    "tkgm.parselsorgu": _src(
        ...
        notes="Public parsel sorgu portalı; çıktı bilgi amaçlı gösterilir, resmi belge gibi sunulmaz.",
    ),
    "tkgm.mevzuat": _src(
        id="tkgm.mevzuat",
    ...
    "csb.bulutkbs": _src(
        ...
        discovery_strategy="public-portal",
        capabilities=["portal", "parcel-query", "layers"],
        notes="Public BulutKBS vatandaş portalı; discovery public görünümleri ve endpoint kontratlarını çözer.",
    ),
    "nvi.maks": _src(
        id="nvi.maks",
        name="MAKS",
...
_MUNICIPAL_SOURCES = [
    ...
    ("bel.uskudar.cbs", "Üsküdar", "https://cbs.uskudar.bel.tr/eharita/", SourceProvider.netcad_webgis),
    ("bel.fatih.webgis", "Fatih", "https://kentrehberi.fatih.bel.tr/webgis/", SourceProvider.netcad_webgis),
    ("bel.batman.keos", "Batman", "https://keos.batman.bel.tr/imardurumu/", SourceProvider.netcad_keos),
    ("bel.avcilar.imar", "Avcılar", "https://www.avcilar.bel.tr/sayfalar/imar-durumu", SourceProvider.netcad_webgis),
...
for _id, _muni, _url, _provider in _MUNICIPAL_SOURCES:
    REGISTRY[_id] = _src(
        ...
        notes=f"{_muni} belediye imar/KEOS portalı; public HTML/JS discovery ile servis uçları aranır ve bilgi amaçlı imar alanları normalize edilir.",
    )
