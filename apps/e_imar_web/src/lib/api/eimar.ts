...
export async function fetchSources() {
  const result = await request<SourcesResponse>(`${API_PROXY_BASE}/sources`);
  return result.ok ? result : { ok: true as const, data: FALLBACK_SOURCES_RESPONSE };
}

export async function fetchSourceHealth() {
  const result = await request<SourceHealthResponse>(`${API_PROXY_BASE}/sources/health`);
...
export async function fetchSourceDetail(sourceId: string) {
  const result = await request<SourceDetailResponse>(`${API_PROXY_BASE}/sources/${encodeURIComponent(sourceId)}`);
