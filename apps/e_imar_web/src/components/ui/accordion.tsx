...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "group flex flex-1 items-center justify-between gap-2 px-4 py-3 text-left",
        "text-sm font-medium text-fg-primary transition-colors hover:bg-surface-1/60",
        "[&[data-state=open]>svg]:rotate-180 outline-none",
        className
      )}
      {...props}
    >
