...
export function GET() {
  ...
  const apiBaseUrl = process.env.NEXT_PUBLIC_EIMAR_API_BASE_URL || process.env.NEXT_PUBLIC_API_BASE_URL;
  ...
  const checks = [
    check("site-url", Boolean(readPublicSiteUrl()), "Canonical website URL can be resolved."),
    check(
      "parcel-source",
      sourceMode === "api" ? Boolean(apiBaseUrl) : sourceMode === "vector-tile" ? Boolean(vectorTileUrl) : !production && sourceMode === "demo",
      sourceMode === "api"
