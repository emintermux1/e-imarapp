...
class ParcelReportPreviewScreen extends StatefulWidget {
  const ParcelReportPreviewScreen({this.parcel = ParcelDetail.sample, super.key});

  final ParcelDetail parcel;

  @override
  State<ParcelReportPreviewScreen> createState() => _ParcelReportPreviewScreenState();
}
...
class _ParcelReportPreviewScreenState extends State<ParcelReportPreviewScreen> {
      ...
      body: ListView(
        ...
        children: [
          ...
          GridView.count(
            ...
            mainAxisSpacing: 10,
            children: [
              MetricCard(title: 'TAKS', value: parcel.taks.toStringAsFixed(2), subtitle: 'Taban oturumu', icon: Icons.square_foot_rounded),
              MetricCard(title: 'KAKS / Emsal', value: parcel.emsal.toStringAsFixed(2), subtitle: 'Yapılaşma hakkı', icon: Icons.functions_rounded),
              MetricCard(title: 'Kat Limiti', value: '${parcel.floorLimit} kat', subtitle: parcel.zoningStatus, icon: Icons.apartment_rounded),
              MetricCard(title: 'Yol Cephesi', value: '${parcel.roadFrontage.toStringAsFixed(1)} m', subtitle: parcel.coverageRatio, icon: Icons.alt_route_rounded),
            ],
          ),
          const SizedBox(height: 14),
          const GlassCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const _InsightRow(icon: Icons.auto_graph_rounded, label: 'Yatırım notu', value: 'Ruhsat, satış hızı ve finansman varsayımları ayrıca doğrulanmalıdır.'),
          ])),
          const SizedBox(height: 14),
          GlassCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 18),
          GradientButton(label: _isGenerating ? 'Rapor hazırlanıyor...' : reportBytes == null ? 'PDF Raporu Oluştur' : 'PDF Yeniden Oluştur', icon: Icons.workspace_premium_rounded, onPressed: _isGenerating ? null : () => _generateReport(metrics)),
          if (reportBytes != null) ...[
            const SizedBox(height: 12),
            GlassCard(child: Row(children: [
              const Icon(Icons.check_circle_rounded, color: AppColors.emerald),
              const SizedBox(width: 12),
              Expanded(child: Text('Rapor bellekte oluşturuldu. Paylaşım eklentisi olmadığı için dosya kaydetmeden ${_formatBytes(reportBytes.length)} üretim bilgisi gösteriliyor.', style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w700))),
            ])),
          ],
        ],
      ),
  ...
  Future<void> _generateReport(Map<String, Object?> metrics) async {
    ...
    final bytes = await _reportService.generateParcelReport(parcelTitle: _parcelTitle(widget.parcel), metrics: metrics);
    if (!mounted) return;
    setState(() {
      _generatedReport = bytes;
      _isGenerating = false;
    });
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Premium PDF raporu hazır: ${_formatBytes(bytes.length)}')));
  }
  ...
  Map<String, Object?> _reportMetrics(ParcelDetail parcel) => {
        ...
        'Risk Özeti': 'Deprem, taşkın ve zemin katmanları için premium analiz yer tutucusu',
        'Piyasa İstihbaratı': 'Yakın çevre değer trendleri mock veriyle temsil edilir',
      };

  String _parcelTitle(ParcelDetail parcel) => '${parcel.city} ${parcel.district} ${parcel.neighborhood} ${parcel.block}/${parcel.parcel}';
  String _formatBytes(int bytes) => bytes < 1024 ? '$bytes B' : '${(bytes / 1024).toStringAsFixed(1)} KB';
}
...
class _SectionTitle extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Row(children: [Icon(icon, color: AppColors.emerald), const SizedBox(width: 10), Text(title, style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w900))]);
}
...
class _InsightRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(icon, color: AppColors.emerald), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label, style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w900)), const SizedBox(height: 3), Text(value, style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: AppColors.slate))]))]),
      );
}
...
class _IncludedSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
        ...
        child: Row(children: [const Icon(Icons.check_circle_rounded, color: AppColors.emerald, size: 20), const SizedBox(width: 10), Expanded(child: Text(section, style: Theme.of(context).textTheme.bodyLarge?.copyWith(fontWeight: FontWeight.w700)))]),
      );
}
...
const _disclaimer = 'Bu rapor bilgilendirme amaçlı önizleme çıktısıdır. Resmi imar durumu belgesi, tapu kayıtları, belediye plan notları ve ilgili kurum görüşleri yerine geçmez. Nihai yatırım, ruhsat ve proje kararları yetkili kurum belgeleriyle doğrulanmalıdır.';
