...
class MockMapCanvas extends StatelessWidget {
  ...
  Widget build(BuildContext context) => const RepaintBoundary(
      child: CustomPaint(painter: _MockMapPainter(), child: SizedBox.expand()));
}
...
class _MockMapPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(
        rect, Paint()..shader = AppGradients.mapMock.createShader(rect));
  ...
  void _drawContours(Canvas canvas, Size size) {
    final paint = Paint()
      ..color = AppColors.mapContour.withOpacity(.09)
    ...
    for (var i = 0; i < 11; i++) {
      ...
      for (var x = 0.0; x <= size.width + 40; x += 44) {
        path.lineTo(
            x, size.height * (.12 + i * .075) + math.sin(x / 68 + i) * 16);
      }
  ...
  void _drawRoads(Canvas canvas, Size size) {
    ...
    final road = Paint()
      ..color = AppColors.mapRoad.withOpacity(.52)
      ..style = PaintingStyle.stroke
      ..strokeWidth = 9
      ..strokeCap = StrokeCap.round;
    final lane = Paint()
    ...
    final main = Path()
      ...
      ..cubicTo(size.width * .28, size.height * .18, size.width * .51,
          size.height * .53, size.width * 1.05, size.height * .39);
    canvas.drawPath(main, casing);
    canvas.drawPath(main, road);
    canvas.drawPath(main, lane);
    final secondary = Paint()
      ..color = const Color(0xFFB8D1BE).withOpacity(.22)
      ..style = PaintingStyle.stroke
      ..strokeWidth = 5
      ..strokeCap = StrokeCap.round;
    for (var i = 0; i < 4; i++) {
      final path = Path()
        ..moveTo(size.width * (.08 + i * .2), size.height * .08)
        ..cubicTo(
            size.width * (.18 + i * .12),
            size.height * .26,
            size.width * (.1 + i * .22),
      ...
      canvas.drawCircle(
          ...
          Paint()
            ..shader = RadialGradient(colors: [
              spot.$2.withOpacity(.25),
              spot.$2.withOpacity(0)
            ]).createShader(Rect.fromCircle(center: spot.$1, radius: spot.$3)));
    }
  }

  void _drawParcels(Canvas canvas, Size size) {
    final fill = Paint()
      ..color = AppColors.mapParcel.withOpacity(.075)
      ..style = PaintingStyle.fill;
    final stroke = Paint()
      ..color = AppColors.mapParcel.withOpacity(.38)
      ..style = PaintingStyle.stroke
      ..strokeWidth = .85;
    for (var row = 0; row < 9; row++) {
      for (var col = 0; col < 7; col++) {
        final cx =
            size.width * (.08 + col * .15) + math.sin(row * 1.7 + col) * 15;
  ...
  void _drawSelectedParcel(Canvas canvas, Size size) {
    ...
    canvas.drawPath(
        path,
        Paint()
          ..color = AppColors.emerald.withOpacity(.23)
          ..maskFilter = const MaskFilter.blur(BlurStyle.outer, 16));
    canvas.drawPath(
        path,
        Paint()
          ..color = AppColors.emerald.withOpacity(.20)
          ..style = PaintingStyle.fill);
    canvas.drawPath(
        path,
        Paint()
          ..color = AppColors.lime.withOpacity(.92)
          ..style = PaintingStyle.stroke
          ..strokeWidth = 2.2);
  }
    ...
    canvas.drawRect(
        ...
        Paint()
          ..shader = RadialGradient(
              colors: [Colors.transparent, Colors.black.withOpacity(.48)],
              stops: const [.58, 1]).createShader(rect));
    canvas.drawRect(
        rect,
        Paint()
          ..shader = const LinearGradient(
              ...
              colors: [Color(0x7A000000), Color(0x00000000), Color(0xA6000000)],
              stops: [0, .43, 1]).createShader(rect));
  }
