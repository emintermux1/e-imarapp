        ...
        builder: (context, controller) => ListView(
          ...
          children: [
            Center(
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: AppColors.slate.withValues(alpha: .24),
            ...
            const ValueScoreCard(
              score: 86,
              title: 'İmar uygunluk özeti',
              subtitle:
                  'Örnek veriye göre yapılaşma koşulları, cephe ve plan yılı uyumlu görünüyor.',
            ),
            ...
            const InsightCard(
              title: 'Plan notu',
              message:
                  'TAKS/KAKS ve kat sınırı örnek plan verisinden gösterilir; resmi işlem öncesi teyit gerekir.',
            ...
            const InsightCard(
              title: 'Etüt notu',
              message:
                  'Ön fizibilite için yol cephesi, çekme mesafeleri ve terk durumu ayrıca incelenmelidir.',
            ...
            Wrap(
              ...
              children: [
                IconActionChip(
                  ...
                  onTap: () => _toggleFavorite(context, ref, parcel),
                ),
                IconActionChip(
                  label: 'Analizi Gör',
                ...
                IconActionChip(
                  ...
                  onTap: () => _toggleFollow(context, ref, parcel),
                ),
                IconActionChip(
                  label: 'Google Earth bağlantısı',
                  icon: Icons.public_rounded,
                  onTap: () => _showGoogleEarthPreview(context),
                ),
                IconActionChip(
                  label: 'Koordinat',
...
void _toggleFavorite(BuildContext context, WidgetRef ref, ParcelDetail parcel) {
  _runUserAction(
    context,
    ref,
    action: (ref, messenger) async {
      ...
      final parcelId =
          '${parcel.neighborhood}_${parcel.block}/${parcel.parcel}';
...
void _toggleFollow(BuildContext context, WidgetRef ref, ParcelDetail parcel) {
  _runUserAction(
    context,
    ref,
    action: (ref, messenger) async {
      ...
      if (followed.contains(parcelId)) {
        ...
        messenger.showSnackBar(
          const SnackBar(content: Text('Takipten çıkarıldı.')),
        );
      } else {
...
void _showGoogleEarthPreview(BuildContext context) {
  const url = 'https://earth.google.com/web/search/40.9758,29.0436';
  Clipboard.setData(const ClipboardData(text: url));
  ScaffoldMessenger.of(context).showSnackBar(
    const SnackBar(
      content: Text(
          'Google Earth önizleme bağlantısı panoya kopyalandı: 40.9758, 29.0436'),
    ),
  );
}

void _copyCoordinates(BuildContext context) {
  const coordinates = '40.9758, 29.0436';
  Clipboard.setData(const ClipboardData(text: coordinates));
  ScaffoldMessenger.of(context).showSnackBar(
    const SnackBar(
        content: Text('Koordinat panoya kopyalandı: 40.9758, 29.0436')),
  );
}
