...
class _Mock3dParcelPreviewState extends State<Mock3dParcelPreview> {
  ...
  Widget build(BuildContext context) {
    ...
    final volumeLabel =
        '${(_floorCount * baseArea).toStringAsFixed(0)} m² (brüt)';
    return GlassCard(
      ...
      child: Column(children: [
        ...
        Text('$_floorCount kat olursa nasıl görünür?',
          ...
          Expanded(
            child: SliderTheme(
              ...
              child: Slider(
                  ...
                  onChanged: (v) => setState(() => _floorCount = v.round())),
            ),
          ),
          _FloorButton(
              icon: Icons.add_rounded,
              onTap: _floorCount < _maxFloor
                  ? () => setState(() => _floorCount++)
                  : null),
        ]),
        ...
        Row(children: [
          Expanded(
              child: Column(
                ...
                Text(
                    '${(widget.parcel.taks * 100).toStringAsFixed(0)}% • ~${baseArea.toStringAsFixed(0)} m²',
                    style: Theme.of(context)
                        .textTheme
                        .bodySmall
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: enabled
              ? AppColors.emerald.withValues(alpha: .16)
              : AppColors.slate.withValues(alpha: .08),
          borderRadius: BorderRadius.circular(12),
          border: Border.all(
              color: enabled
                  ? AppColors.emerald.withValues(alpha: .4)
                  : AppColors.slate.withValues(alpha: .15)),
        ),
        child: Icon(icon,
            size: 22,
            color: enabled
                ? AppColors.emerald
                : AppColors.slate.withValues(alpha: .3)),
      ),
    );
  }
}
...
class _MockMassingPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawPath(
        plotPath,
        Paint()
          ..color = AppColors.slate.withValues(alpha: .07)
          ..style = PaintingStyle.fill);
    canvas.drawPath(
        plotPath,
        Paint()
          ..color = AppColors.slate.withValues(alpha: .18)
    ...
    for (var f = 0; f < floorCount && f < 30; f++) {
      ...
      final bottom = Path()
        ...
        ..lineTo(bldOrigin.dx + bldW * .30,
            bldOrigin.dy - yOff - bldW * .14 - bldH / 30)
      ...
      final left = Path()
        ...
        ..lineTo(bldOrigin.dx - bldW * .30, bldOrigin.dy - yOff - bldH / 30)
        ..lineTo(bldOrigin.dx - bldW * .30,
            bldOrigin.dy - yOff - bldH / 30 - bldH / 30)
        ..lineTo(bldOrigin.dx - bldW * .32, bldOrigin.dy - yOff - bldH / 30)
        ..close();
      canvas.drawPath(
          left,
          Paint()
            ..color = AppColors.forest.withValues(alpha: opacity * .6)
            ..style = PaintingStyle.fill);
      ...
      final right = Path()
        ...
        ..lineTo(bldOrigin.dx + bldW * .30,
            bldOrigin.dy - yOff - bldW * .14 - bldH / 30)
        ..lineTo(bldOrigin.dx + bldW * .30,
            bldOrigin.dy - yOff - bldW * .14 - bldH / 30 - bldH / 30)
        ..lineTo(bldOrigin.dx + bldW * .28,
            bldOrigin.dy - yOff - bldW * .14 - bldH / 30)
        ..close();
      canvas.drawPath(
          right,
          Paint()
            ..color = AppColors.deepGreen.withValues(alpha: opacity * .5)
            ..style = PaintingStyle.fill);
    }
    ...
    canvas.drawPath(
        roof,
        Paint()
          ..color = AppColors.lime.withValues(alpha: .7)
          ..style = PaintingStyle.fill);
    canvas.drawPath(
        roof,
        Paint()
    ...
    canvas.drawPath(
        plotPath,
        Paint()
          ..color = AppColors.slate.withValues(alpha: .22)
          ..style = PaintingStyle.stroke
          ..strokeWidth = 1.2);

    _drawDot(
        canvas, Offset(bldOrigin.dx - bldW * .36, bldOrigin.dy), 'Yol', false);
    _drawDot(
        canvas,
        Offset(bldOrigin.dx + bldW * .36, bldOrigin.dy - bldW * .18),
        'K',
        true);
  }
  ...
  void _drawDot(Canvas canvas, Offset pos, String label, bool rightSide) {
    canvas.drawCircle(
        pos, 4, Paint()..color = AppColors.sand.withValues(alpha: .6));
  ...
  @override
  bool shouldRepaint(covariant _MockMassingPainter oldDelegate) =>
