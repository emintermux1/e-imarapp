...
class _HomeMapScreenState extends ConsumerState<HomeMapScreen>
  ...
  void _openParcel() {
    ...
    showModalBottomSheet<void>(
            ...
            builder: (_) =>
                const ParcelDetailSheet(parcel: ParcelDetail.sample))
...
class _MapSurface extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (!config.hasMapbox)
      return GestureDetector(onTap: onTap, child: const MockMapCanvas());
    mapbox.MapboxOptions.setAccessToken(config.mapboxAccessToken);
    return RepaintBoundary(
        child: mapbox.MapWidget(onTapListener: (_) => onTap()));
  }
}
...
class _LayerStack extends StatelessWidget {
  const _LayerStack(
      {required this.onMap, required this.onTerrain, required this.onThreeD});
...
class _RoundIcon extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
      ...
      height: 36,
      decoration: BoxDecoration(
          color: AppColors.emerald.withOpacity(.12), shape: BoxShape.circle),
