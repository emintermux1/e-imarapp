...
class _ParcelSearchScreenState extends ConsumerState<ParcelSearchScreen> {
    ...
    return Scaffold(
      ...
      body: DecoratedBox(
        ...
        child:
          ...
          PremiumHeader(
              title: isParcel ? 'Bölgesel Akıllı Arama' : 'GPS ile Parsel Bul',
              subtitle: isParcel
                  ? 'İl, ilçe, mahalle ve ada/parsel ile mock sorgu deneyimi.'
                  : 'Türkiye sınırları içinde koordinat doğrulaması ve parsel önizlemesi.',
              icon: isParcel
                  ? Icons.travel_explore_rounded
                  : Icons.gps_fixed_rounded,
              badge: isParcel ? 'Ada / Parsel' : 'Koordinat'),
          ...
          if (validation != null)
            Padding(
                ...
                child: _ValidationBanner(
                    message: validation!, isError: validationIsError)),
          const SizedBox(height: 16),
          GradientButton(
              label: isParcel ? 'Parseli Sorgula' : 'Koordinatı Doğrula',
          ...
          for (final item in const [
            ('İstanbul / Kadıköy / Fenerbahçe', '1247/18 • yatırım skoru 86'),
            ('Ankara / Çankaya / Alacaatlı', '8912/4 • konut gelişim alanı'),
            ('İzmir / Urla / Kuşçular', 'Koordinat • sahil etkisi')
          ])
            Padding(
  ...
  void _validate() {
    setState(() {
      if (mode == SearchMode.parcel &&
          (block.text.trim().isEmpty || parcel.text.trim().isEmpty)) {
        ...
        final invalid = la == null ||
            ...
            lo > 45.0;
        validation = invalid
            ? 'Koordinatlar Türkiye sınırları içinde olmalıdır.'
            : 'Koordinat doğrulandı. Mock parsel sonucu hazır.';
  ...
  void _saveQuery() {
    ...
    if (mode == SearchMode.parcel) {
      query =
          'İstanbul / Kadıköy / Fenerbahçe ${block.text.trim()}/${parcel.text.trim()}';
    } else {
...
class _ParcelForm extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GlassCard(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const _Selector(
            label: 'İl', value: 'İstanbul', icon: Icons.location_city_rounded),
        const SizedBox(height: 10),
        const _Selector(
...
class _CoordinateForm extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GlassCard(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 10),
        const InsightCard(
            title: 'Doğrulama aralığı',
            message: 'Türkiye bounds: enlem 35.8–42.2, boylam 25.5–45.0',
