...
import { BadgeCheck, ClipboardCopy, Globe2, Link2, Loader2, ShieldAlert, Sparkles } from "lucide-react";
    ...
    <div className="space-y-6 animate-fade-in-up">
      <section className="rounded-[28px] border border-[var(--border-subtle)] bg-[radial-gradient(circle_at_top_right,rgba(0,229,255,0.16),transparent_34%),linear-gradient(180deg,rgba(17,17,24,0.95),rgba(10,10,15,0.96))] p-6 md:p-8 shadow-[0_20px_80px_rgba(0,0,0,0.35)] overflow-hidden">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="space-y-3 max-w-2xl">
            ...
            <h1 className="text-3xl md:text-4xl font-bold tracking-tight">Kaynak URL’si için normalizasyon önizlemesi</h1>
            <p className="max-w-2xl text-sm md:text-base text-[var(--text-secondary)] leading-6">
              Bir belediye veya kaynak URL’si yapıştırın; sistemin bunu nasıl sınıflandıracağını, hangi connector’ları önereceğini ve hangi erişim durumunu
              tahmin edeceğini görün. Bu ekran bir kayıt oluşturmaz.
            </p>
          </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.04fr_0.96fr]">
        <form onSubmit={submit} className="space-y-5 rounded-[28px] border border-[var(--border-subtle)] bg-[var(--bg-card)] p-6 shadow-[0_16px_50px_rgba(0,0,0,0.25)]">
          ...
          {error && <div className="rounded-2xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-100">{error}</div>}
        ...
        <aside className="space-y-4 rounded-[28px] border border-[var(--border-subtle)] bg-[var(--bg-card)] p-6 shadow-[0_16px_50px_rgba(0,0,0,0.25)]">
          ...
          {!result ? (
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-2xl border border-[var(--border-subtle)] bg-white/[0.03] p-4">
                <div className="text-[11px] uppercase tracking-[0.18em] text-[var(--text-secondary)]">wouldRegister preview</div>
                <pre className="mt-3 overflow-x-auto rounded-xl bg-black/20 p-3 text-[11px] leading-5 text-[var(--text-secondary)]">{JSON.stringify(seedPreview, null, 2)}</pre>
              </div>
            </div>
          )}
        </aside>
