...
export class WebsiteService {
  ...
  async parcelReport(input: {
    ...
    const parcelWorkflow = input.parcelWorkflow ?? (
      typeof this.parcels?.queryParcel === 'function'
        ? await this.parcelWorkflow({
            query: {
        ...
        : { status: 'not_ready', parcelQuery: { status: 'not_ready', parcels: [], count: 0 }, potentialSummary: { status: 'not_ready', summary: {} }, emsalShare: null }
    );
    const municipalWorkflow = input.municipalWorkflow ?? (
      typeof this.sources?.findMunicipality === 'function'
        ? await this.municipalParcelWorkflow({
