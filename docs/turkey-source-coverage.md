...
- `metadata_only`: the registry contains a pattern-derived candidate or discovery seed, not a verified live service.
...
- Broad municipal coverage candidates derived from known portal patterns such as Netcad KEOS, WebGIS, eKent, municipal KBS portals, and public metadata-only seeds.
...
- provinces that are still metadata-only,
...
If the registry says a source is `metadata_only`, `public_metadata`, or `requires_legal_agreement`, the downstream workflow must preserve that state and show the appropriate next action instead of returning fabricated parcel, zoning, or plan data.
